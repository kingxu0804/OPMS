...
<body class="sticky-header">
	...
	<div class="main-content">
		<div class="header-section">
			...
			<form class="searchform" action="searchExpense" method="post">
		...
		<div class="page-heading">
			...
			<div class="pull-right">
				<a href="#" class="hidden-xs btn btn-default">全部</a> <a
					href="approvalexpense" class="btn btn-warning"
					style="padding: 4px;">待审批</a> <a href="addexpensemanage"
					class="btn btn-success">+我要报销</a>
			</div>
		</div>
		<div class="wrapper">
			<div class="row">
				<div class="col-sm-12">
					...
					报销 / 总数：${listExpense.size()}<span class="tools pull-right"><a
							   ...
							   <c:if test="${listExpense.size()==0 }">
                                ...
                                <c:if test="${listExpense.size()>0 }">
									<c:forEach items="${listExpense}" var="expense">
										<tr>
										<td class="hidden-phone"><fmt:formatDate value="${expense.created }" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
										<td><span class="label label-warning label-mini"><c:if
													test="${expense.status==1 }">草稿</c:if>
												<c:if test="${expense.status==2 }">正常发布</c:if></span></td>
										<td><span class="label label-warning label-mini"><c:if test="${expense.result==1}">同意</c:if><c:if test="${expense.result==2}">拒绝</c:if> <c:if
													test="${expense.result==null}">等待中</c:if> </span></td>
										<td>
											<div class="js-selectuserbox">
												<a href="javascript:;" title="张三"> <img class="gray"
													src="img/3.jpg" alt="张三">未处
												</a> <span>..........</span> <a href="javascript:;" title="lock">
													<img class="gray" src="img/1.jpg" alt="lock">未处
												</a> <span>..........</span> <a href="javascript:;" title="李浩">
													<img class="gray" src="img/1.jpg" alt="李浩">未处
												</a> <span>..........</span> <a href="javascript:;" title="李四">
													<img class="gray" src="img/2.jpg" alt="李四">未处
												</a> <span>..........</span> <a href="javascript:;" title="朱笑天">
													<img class="gray" src="img/1.jpg" alt="朱笑天">未处
												</a>
											</div>
										</td>
										<td>
										<c:if test="${ expense.status==1}">
											<div class="btn-group">
												...
												<ul class="dropdown-menu">
													<li><a href="editexpensemanage?expenseid=${expense.expenseid}">编辑</a></li>
													<li role="separator" class="divider"></li>
													<li><a href="delexpenseinfo?expenseid=${expense.expenseid}" class="js-expense-delete"
														data-op="delete" data-id="100344740200845312">删除</a></li>
													<li role="separator" class="divider"></li>
													<li><a href="updateexpensestatus?expenseid=${expense.expenseid}" class="js-expense-status"
														data-op="status" data-id="100344740200845312">正常</a></li>
												</ul>
											</div>
											</c:if><c:if test="${ expense.status==2}">
											 <a href="expensedetail?expenseid=${expense.expenseid}"> 查看 </a>  </td>
											</c:if>
										</td>
