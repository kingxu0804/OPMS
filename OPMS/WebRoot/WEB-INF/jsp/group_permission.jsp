...
<head>
    ...
    <style>
        ...
        .border { border-bottom: 1px solid rgb(221, 221, 221); margin-bottom: 20px; }
        .pul {  }
        .pul li { display: block; float: left; width: 100%; }
        .cul { display: block; }
        .cul li { float: left; width: 130px; }
    </style>
</head>
                ...
                <div class="col-sm-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <section id="unseen">
                                <form id="group-permission-form" novalidate="novalidate" action="setGroupPermission">
                                    <ul class="list-unstyled pul" >
                                    	<c:forEach items="${listall }" var="permission" varStatus="ind">
                                    		<c:if test="${ind.index==0&&permission.isexist==null }">
                                    			<li data-pmodel="${permission.name }-${permission.ename}" class="border">
												<h4>${permission.name} <input name="checkpermission" type="checkbox"></h4>
												<ul class="cul">
                                    		</c:if>
                                    		<c:if test="${ind.index!=0&&permission.isexist==null }">
                                    			</ul>
                                    		...
                                    		<c:if test="${ind.index!=0&&permission.isexist==1 }">
                                    			<li>
												<div class="form-group" data-cmodel="${permission.name}"> 
													<label class="checkbox-inline">
													<input name="permissions" data-ename="${permission.ename}" value="${permission.permissionid}" checked="checked" type="checkbox">
													${permission.name}</label>
												</div>
												</li>
                                    		</c:if>
                                    		<c:if test="${ind.index!=0&&permission.isexist==0 }">
                                    			<li>
												<div class="form-group" data-cmodel="${permission.name}"> 
													<label class="checkbox-inline">
													<input name="permissions" data-ename="${permission.ename}" value="${permission.permissionid}" type="checkbox">
													${permission.name}</label>
												</div>
												</li>
                                    		</c:if>
										...
										<li class="text-center">
                                            <input  name="groupid" value="${group.groupid }" type="hidden">
                                            <button type="submit" id="permission-btn-new" class="btn btn-success">权限设置</button>
                                        </li>
                                    </ul>
                                </form>
                            </section>
