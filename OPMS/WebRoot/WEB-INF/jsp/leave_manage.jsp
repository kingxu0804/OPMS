...
<body class="sticky-header">
	...
	<div class="main-content">
		...
		<div class="header-section">
			...
			<form class="searchform" action="searchLeaveInfo" method="post">
				...
				<ul class="notification-menu">
					...
					<li><a href="javascript:;"
						...
						<ul class="dropdown-menu dropdown-menu-usermenu pull-right">
							...
							<li><a href="tologout"> <i class="fa fa-sign-out"></i> 退出
							</a></li>
						</ul></li>
				</ul>
			</div>
		...
		<div class="page-heading">
			<h3>
				审批管理 <span> <a href="leavemanage">请假</a> <a
					href="overtimemanage">加班</a> <a href="expensemanage">报销</a>
					<a href="businesstripmanage">出差</a> <a
					href="gooutmanage">外出</a> <a href="oagood_manage">物品</a>
				</span>
			</h3>
			...
			<div class="pull-right">
				<a href="" class="btn btn-default hidden-xs"> 全部 </a> <a
					href="approvalleave" class="btn btn-warning" style="padding: 4px;">
					待审批 </a> <a href="goaddleavemanage" class="btn btn-success"> +我要请假 </a>
			</div>
		</div>
		<div class="wrapper">
			<div class="row">
								...
								<c:forEach items="${listleave}" var="leave" varStatus="i">
									<tr>
										<td><c:if test="${leave.type==1}">事假</c:if> <c:if
												...
												test="${leave.type==3}">年假</c:if> <c:if
												test="${leave.type==4}">调休</c:if> <c:if
												test="${leave.type==5}">婚假</c:if> <c:if
												test="${leave.type==6}">产假</c:if> <c:if
												test="${leave.type==7}">陪产假</c:if> <c:if
												test="${leave.type==8}">路途假</c:if> <c:if
												test="${leave.type==9}">其他</c:if></td>
										<td class="hidden-phone hidden-xs"><fmt:formatDate
												value="${leave.started}" type="date" pattern="yyyy-MM-dd" />至<fmt:formatDate
												value="${leave.ended }" type="date" pattern="yyyy-MM-dd" /></td>
										<td><fmt:formatNumber value="${leave.days }" pattern="0" />天
										</td>
										<td><span class="label label-warning label-mini"><c:if
													test="${leave.status==1}">草稿</c:if>
												<c:if test="${leave.status==2}">正常发布</c:if></span></td>
										<td><span class="label label-warning label-mini"><c:if
													test="${leave.result==1}">同意</c:if>
												<c:if test="${leave.result==2}">拒绝</c:if> <c:if
										...
										<td>
												<div class="btn-group">
													...
													<ul class="dropdown-menu">
														<li><a
															href="editleavemanage?leaveid=${leave.leaveid}">编辑</a></li>
														<li role="separator" class="divider"></li>
														<li><a href="deleteLeave?leaveid=${leave.leaveid}" class="js-leave-delete"
															data-op="delete" data-id="103932084682231808">删除</a></li>
														<li role="separator" class="divider"></li>
														<li><a href="updateLeaveStatus?leaveid=${leave.leaveid}" class="js-leave-status"
															data-op="status" data-id="103932084682231808">正常</a></li>
													</ul>
												</div>
											</c:if> <c:if test="${leave.status==2}">
												<td><a href="leavedetail?leaveid=${leave.leaveid}"> 查看 </a></td>
											</c:if></td>
									</tr>
								</c:forEach>
							</c:if>
