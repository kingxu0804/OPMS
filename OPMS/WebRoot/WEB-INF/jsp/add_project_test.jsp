                        ...
                        <div class="panel-body">
                            <form action="addBug" method="post" enctype="multipart/form-data" class="form-horizontal adminex-form" id="test-form" novalidate="novalidate">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="needsid" class="form-control" id="needsid">
                                            <c:forEach items="${listneeds}" var="need">
                                                 <option value="${need.needsid}">${need.name}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input name="name" value="" class="form-control" placeholder="请输入测试名称" type="text">
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <c:forEach items="${listTeam}" var="team">
                    ...
                    <input data-value="${team.userid}" data-name="${team.username}" type="checkbox"> ${team.username} </label>
                 </c:forEach>
            </div>
...
<script>
    ...
    var need=document.getElementById("needsid");
    need.onchange=function(){
		...
		httpRequest.onreadystatechange=function(){
			if(httpRequest.readyState==4 & httpRequest.status==200){
				...
				for(var i=0;i<obj.length;i++){
					...
					op.text=obj[i].name;
					op.value=obj[i].taskid;
					task.appendChild(op);
				}
			}
		}
		httpRequest.open("POST","listProjectsTask?needsid="+need.value,true);
